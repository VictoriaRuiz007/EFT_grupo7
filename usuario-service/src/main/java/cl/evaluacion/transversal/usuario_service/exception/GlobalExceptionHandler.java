...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> manejarNoEncontrado(UsuarioNoEncontradoException ex, HttpServletRequest request){
    ...
    public ResponseEntity<ApiError> manejarExistente(UsuarioExisteException ex, HttpServletRequest request){
        ApiError error = new ApiError(LocalDateTime.now(), 409, "YA EXISTE", List.of(ex.getMessage()), request.getRequestURI() );
        return ResponseEntity.status(HttpStatus.CONFLICT).body(error);
    }

    @ExceptionHandler(CorreoNoEncontradoException.class)
    public ResponseEntity<ApiError> manejarCorreoNoEncontrado(CorreoNoEncontradoException ex, HttpServletRequest request){
        ApiError error = new ApiError(LocalDateTime.now(),404, "CORREO NO ENCONTRADO", List.of(ex.getMessage()), request.getRequestURI());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(error);
    }
