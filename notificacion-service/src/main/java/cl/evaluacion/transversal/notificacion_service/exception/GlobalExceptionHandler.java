...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> manejarNoEncontrado(NotificacionNoEncontradaException ex, HttpServletRequest request){
    ...
    public ResponseEntity<ApiError> manejarExistente(UsuarioNoEncontradoException ex, HttpServletRequest request){
        ApiError error = new ApiError(LocalDateTime.now(), 404, "NO ENCONTRADO", List.of(ex.getMessage()), request.getRequestURI() );
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(error);
    }
