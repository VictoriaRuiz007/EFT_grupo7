...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> manejarNoEncontrado(TransaccionNoEncontradaException ex, HttpServletRequest request){
    ...
    public ResponseEntity<ApiError> manejarPedidoNoEncontrado (PedidoNoEncontradoException ex, HttpServletRequest request){
        ApiError error = new ApiError(LocalDateTime.now(), 404, "PEDIDO NO ENCONTRADO", List.of(ex.getMessage()), request.getRequestURI());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(error);
    }
}
